...
def find_language_facts(languages, language_name, fact_index = 0)
  ...
  # }
  # 1st attempt:
  # languages[language_name][:facts][fact_index] --> doesn't help when a language_name is missing/doesn't exist
  # 2nd attempt - for easy debugging!!!:
  # facts_array = languages.dig(language_name, :facts) --> nil if any of the two is missing,
  # facts_array[fact_index] if facts_array --> nil if there's no fact_array
  # pretty attempt - provides for nill values but not as easy to debug:
